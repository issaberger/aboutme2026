...
const NewsModule = () => {
  ...
  const fetchIntel = async (category: string) => {
    ...
    try {
      ...
      // Extract items from AI text blocks
      const items: NewsItem[] = text.split('\n\n')
        .filter(t => t.length > 30)
        .map((block, i) => {
          const lines = block.split('\n');
          return {
            title: lines[0]?.replace(/^#+\s*|\d+\.\s*/, '').trim() || "Intercepted Signal",
            summary: lines.slice(1).join(' ').substring(0, 180).trim() + "...",
            source: "Global Intelligence",
            url: sources[i] || "https://google.com/search?q=" + encodeURIComponent(lines[0]),
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-black font-sans">
      ...
      <div className="bg-primary/5 border-b border-primary/20 py-2 overflow-hidden flex items-center whitespace-nowrap z-20">
        ...
        >
          {(news.length > 0 ? news : [{ title: "ESTABLISHING SECURE CONNECTION TO WORLDWIDE TECH GRID..." }]).map((n, i) => (
      ...
      <div className="flex-1 overflow-hidden flex flex-col md:flex-row relative">
        ...
        <div className="w-full md:w-64 border-b md:border-b-0 md:border-r border-gray-800 p-4 shrink-0 bg-black/40 z-10 backdrop-blur-md">
          <div className="flex md:flex-col overflow-x-auto md:overflow-x-visible no-scrollbar gap-2 pb-2 md:pb-0">
            <h3 className="hidden md:block text-[10px] font-bold text-gray-600 uppercase tracking-[0.4em] mb-4 px-2">Bandwidth_Frequencies</h3>
            {CATEGORIES.map((cat) => (
              <button
                key={cat.id}
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10 scroll-smooth">
            ...
            {loading ? (
              ...
              >
                ...
                <div className="space-y-2">
                   ...
                   <p className="text-[10px] md:text-xs text-primary/40 font-mono">SECURE_LINK_STABLISHED // BYPASSING NODE_419 // RECV_BUFFER_ACTIVE</p>
                </div>
              ...
              >
                 ...
                 <CyberButton onClick={() => fetchIntel(activeCat)} variant="danger">RE-INITIALIZE PROTOCOL</CyberButton>
              </motion.div>
            ) : (
              <motion.div 
                key="content"
          ...
          {/* Persistent Action Bar (Mobile Bottom) */}
